...
function getLegacyThemeForHour(hour: number): {
  sky: RGB;
  cloud: RGB;
  cloudShadow: RGB;
} {
...
function getThemeForHour(hour: number): {
  ...
  if (clamped < ANCHOR_HOUR_BRIGHT_SHIFT) {
    ...
    return {
      ...
      cloud: lerpRGB(
        lockedAnchors.night.cloud,
        lockedAnchors.brightShift.cloud,
        t,
      ),
      cloudShadow: lerpRGB(
...
export function initVanta(): void {
  ...
  // The cloud canvas is persisted across view transitions (see VantaBg.astro),
  // so window-level listeners are bound exactly once. astro:after-swap
  // re-applies the scroll transform when the new page's content height
  // changes.
