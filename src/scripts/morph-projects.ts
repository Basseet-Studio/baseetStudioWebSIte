...
function attachMorph(card: HTMLElement, targetD: string): void {
  ...
  // ── Pick the morph source: skip the backdrop, choose the most complex
  //    remaining path. ────────────────────────────────────────────────
  // The Phosphor backdrop is the 256x256 rect with no fill/stroke that
  // sits behind every icon. After convertToPath it's a 256x256 box
  // outline — if we picked it as the morph source, the user would see
  // "a box morphing into the name" instead of the actual icon. Filter
  // it out, then pick the path with the LONGEST d attribute (most
  // points/segments = most icon-like shape).
  ...
  const candidates = paths.filter((p) => !isBackdrop(p))
  ...
  try {
    ...
    log(label, 'primary chosen: d length =', (primary.getAttribute('d') || '').length)
  }
  ...
  const decor: SVGElement[] = [
    ...(paths.filter((p) => p !== primary) as SVGElement[]),
    ...Array.from(
      iconSvg.querySelectorAll<SVGElement>('line, rect, circle, polygon, polyline'),
    ),
  ]
...
function playMorph(m: CardMorph, dir: 'in' | 'out', label: string): void {
  ...
  log(label, 'playMorph', dir, '→ toD length:', toD.length, '· decor→', fadeDecor)
...
async function init(): Promise<void> {
  ...
  const cards = Array.from(stage.querySelectorAll<HTMLElement>('[data-px-card]'))
