        ...
        return new Promise((resolve, reject) => {
            ...
            const attemptLoad = () => {
                loader.load(
                    ...
                    // Error callback
                    (error) => {
                        console.warn(`Failed to load texture ${path} (attempt ${retryCount + 1}/${maxRetries + 1}):`, error);
                        
                        if (retryCount < maxRetries) {
                            retryCount++;
                            console.log(`Retrying texture load: ${path}`);
                            setTimeout(attemptLoad, 1000);
                        } else {
                            // Create fallback texture
                            console.warn(`Using fallback texture for channel ${channelIndex}`);
